...
#define MAX_AV_OUTPUT 8192
static char output[MAX_AV_OUTPUT];

typedef struct {
    unsigned int count[2], state[4];
...
void MD5Update(MD5_CTX *context, const unsigned char *input, unsigned int inputLen) {
    ...
    if ((context->count[0] += ((unsigned int)inputLen << 3)) < ((unsigned int)inputLen << 3)) context->count[1]++;
...
void MD5_memset(unsigned char *output, int value, unsigned int len) {
    for (unsigned int i = 0; i < len; i++) output[i] = (unsigned char)value;
}
...
void md5_to_hex(const unsigned char* hash, char* hex) {
    for (int i = 0; i < 16; i++) {
...
const char* avatar_lookup(const char* email, const char* alias, const char* first, const char* last) {
    memset(output, 0, MAX_AV_OUTPUT);
    strncat(output, "=== Avatar / Profile Image Intelligence ===\n\n", MAX_AV_OUTPUT - strlen(output) - 1);

    // Gravatar section
    if (email && strlen(email) > 0) {
        char normalized[256] = {0};
        strncpy(normalized, email, 255);
        to_lowercase(normalized);

        unsigned char hash[16];
        MD5_CTX ctx;
        MD5Init(&ctx);
        MD5Update(&ctx, (const unsigned char*)normalized, strlen(normalized));
        MD5Final(hash, &ctx);
        ...
        char grav[1024];
        snprintf(grav, sizeof(grav),
            "🧠 Gravatar:\n"
            "[+] Profile: https://www.gravatar.com/%s\n"
            "[+] Avatar : https://www.gravatar.com/avatar/%s\n\n"
            "🔍 Reverse Image Search:\n"
            "[+] Google: https://www.google.com/searchbyimage?image_url=https://www.gravatar.com/avatar/%s\n"
            "[+] TinEye: https://tineye.com/search?url=https://www.gravatar.com/avatar/%s\n"
            "[+] Yandex: https://yandex.com/images/search?rpt=imageview&url=https://www.gravatar.com/avatar/%s\n\n",
            hex, hex, hex, hex, hex
        );
        strncat(output, grav, MAX_AV_OUTPUT - strlen(output) - 1);
    }

    // Username-based public avatars
    if ((alias && strlen(alias) > 0) || (first && last && strlen(first) > 0 && strlen(last) > 0)) {
        char username[128] = {0};
        if (alias && strlen(alias) > 0) {
            snprintf(username, sizeof(username), "%s", alias);
        } else {
            snprintf(username, sizeof(username), "%s%s", first, last);
        }

        char reuse[512];
        snprintf(reuse, sizeof(reuse),
            "🔁 Avatar Reuse Links:\n"
            "[+] GitHub PFP:   https://github.com/%s.png\n"
            "[+] Reddit JSON:  https://www.reddit.com/user/%s/about.json\n"
            "[+] Twitter PFP:  https://twitter.com/%s/profile_image?size=original\n"
            "[+] Instagram:    https://www.instagram.com/%s\n",
            username, username, username, username
        );
        strncat(output, reuse, MAX_AV_OUTPUT - strlen(output) - 1);
    }
