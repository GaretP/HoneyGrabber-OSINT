document.addEventListener('DOMContentLoaded', () => {
  ...
  function runFriendMap() {
    const input = getInputValues();
    return ccallSafe(window.FriendMapModule, 'map_friends', 'string', ['string', 'string'], [input.alias, input.email]);
  }
  ...
  function runSearchEngine() {
    const input = getInputValues();
    const full = (input.first + ' ' + input.last).trim();
    return ccallSafe(window.SearchModule, 'search_engine_dork', 'string', ['string', 'string'], [full, input.alias]);
  }
